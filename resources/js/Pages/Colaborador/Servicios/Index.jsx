...
/* ─── Main ────────────────────────────────────────────────────── */
export default function ColaboradorServicios() {
  const { props } = usePage()
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        {/* Header */}
        <div>
          <p className="text-xs font-semibold text-violet-400 uppercase tracking-widest mb-1 flex items-center gap-1.5">
            <TruckIcon className="w-3.5 h-3.5" />
            Colaborador
          </p>
          <h1 className="text-2xl font-extrabold text-white">Mis Fletes</h1>
          <p className="text-sm text-slate-400 mt-0.5">
            Fletes en los que has participado. Vista de solo lectura.
          </p>
        </div>
        ...
        {lista.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 rounded-2xl bg-violet-500/10 border border-violet-400/20 flex items-center justify-center mb-4">
              <ArrowPathIcon className="w-8 h-8 text-violet-400/40" />
            </div>
            <p className="text-slate-300 font-semibold">Sin fletes asignados</p>
            <p className="text-slate-500 text-sm mt-1">Los fletes donde participaste aparecerán aquí.</p>
          </div>
        ) : (
