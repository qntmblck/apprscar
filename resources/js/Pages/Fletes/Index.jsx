...
import { ArrowPathIcon, DocumentArrowDownIcon, CheckCircleIcon, TruckIcon, XCircleIcon } from '@heroicons/react/20/solid'
  ...
  return (
    <AuthenticatedLayout user={auth.user}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
        ...
        <h1 className="text-2xl font-extrabold text-white">Fletes</h1>
      </div>

      {/* Notification banners */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 space-y-2">
        {successMensaje && (
          <div className="flex items-start gap-3 text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 text-sm">
            <CheckCircleIcon className="w-4 h-4 shrink-0 mt-0.5" />
            <span>{successMensaje}</span>
          </div>
        )}
        {errorMensaje && (
          <div className="flex items-start gap-3 text-red-300 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3 text-sm">
      ...
      {selectedIds.length > 0 && (
        <div className="fixed top-0 inset-x-0 z-50 flex justify-end py-2 pr-20">
          <div className="bg-[#0c1e3a]/90 backdrop-blur border border-[#0094d9]/30 shadow-xl rounded-xl px-3 py-1.5 flex items-center gap-1.5">
            <span className="text-[10px] font-semibold text-[#0094d9] mr-1">{selectedIds.length} seleccionado{selectedIds.length !== 1 ? 's' : ''}</span>
            <button
              onClick={handleClearSelection}
              className="p-1.5 hover:bg-white/10 rounded-lg transition-colors"
              title="Limpiar selección"
            >
              <ArrowPathIcon className="h-4 w-4 text-red-400" />
            </button>
            <button
            ...
            >
              <CheckCircleIcon className="h-4 w-4 text-[#0094d9]" />
            </button>
            <button
            ...
            <button
              ...
              className={`flex items-center px-2.5 py-1 text-xs font-semibold text-white rounded-lg transition-colors ${
                isLoadingResumen ? 'bg-[#0094d9]/40 cursor-not-allowed' : 'bg-[#0094d9]/70 hover:bg-[#0094d9]'
              }`}
            >
            ...
            <button
              ...
              className="px-2.5 py-1 text-xs font-semibold bg-emerald-500/20 hover:bg-emerald-500/40 border border-emerald-500/30 text-emerald-300 rounded-lg transition-colors"
            >
